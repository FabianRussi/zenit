...
,	uploadExtension: function uploadExtension(data, cb) {
        const credentials = nconf.get('credentials');
        const file = credentials_inquirer.nsdeploy_path;
        const files = [];

        const {script, deploy } = nconf.get('services:DEPLOY_SERVICE');

		const deployPath = path.join(process.gulp_init_cwd, nconf.get('folders:deploy'));
        const toDeploy = glob(deployPath + '/**/*.*', { nodir: true });
        const chunk = nconf.get('deploy_config:chunk_size') || 100;
        ...
        const tasks = files.map((page, index) => {
            return function(cb)
            {
                ...
                options.target_folder = {
                    parentId: nconf.get('folders:extmech_parent'),
                    folderName: nconf.get('folders:extensions_dest_name')
                };
