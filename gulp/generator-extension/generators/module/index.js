...
module.exports = class extends Generator {
  ...
  _inquireExtension() {
    const config = this.fs.readJSON(this.config_path);
    let { extensions_path } = config.folders;
    let manifest;
    let manifest_path;
    const self = this;
    ...
    if (extensions_path.length) {
      ...
      const extensions = _.map(extensions_path, (path) => path.replace(`${config.folders.source.source_path}/`, ''),);
      ...
      return this.prompt([
        ...
        manifest_path = path.join(
          this.contextRoot,
          config.folders.source.source_path,
          ext_answer,
          'manifest.json',
        );
